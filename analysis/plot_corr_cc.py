    ...
    if task == "CR":
        ...
        for i in os.listdir(data_dir_ceval):
            if i in corrrect_id_list_cevel:
                ...
                overall_results[i] = 0
        for i in os.listdir(data_dir_avatar):
            if i in corrrect_id_list_avatar:
                overall_results[i] = 1
            else:
                overall_results[i] = 0
        for i in os.listdir(data_dir_crux):
            if i in corrrect_id_list_crux:
                ...
                overall_results[i] = 0
    overall_cc = {}
    path_cc = "../Experiment_Results/result_stat/CC/cc.json"
...
def plot_corr(model, task):
    overall_results, overall_cc = load_data(task, model)
    path = f"../Experiment_Results/figures/program_complexity/{task}_{model}.jpg"
    plotter = plot_bar_only(overall_results, overall_cc, model, path, "CC")
    plotter.plot()

if __name__ == "__main__":
    plot_corr('gpt-4-turbo', "IER")
    plot_corr('gemini-1.5-pro', 'IER')
    plot_corr('semcoder_s','IER')
    plot_corr('starcoder2-15b', 'IER')
    plot_corr('deepseek-coder-33b-instruct', 'IER')

# load_data("SR", "semcoder_s")